...
public class DisplayCars extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            for(int i=1; i<6;i++)
            {
                out.println("<h3>"+String.valueOf(i)+". "+controller.getMake(i)+" - "+controller.getModel(i)
                        +" - "+String.valueOf(controller.getYear(i))+" - "+String.valueOf(controller
                                .getMileages(i).get(controller.getMileages(i).size()-1).getMileage())+"km - "+String
                                        .valueOf(controller.getPrice(i))+ "zł/h </h3>");     
            }
