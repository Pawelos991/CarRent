...
public class Car {
        ...
        for(Mileage add : added)
        {
            if(add!=null)
            {
                ...
                if(latest.getDate().isLaterThan(add.getDate()))
                    throw new MileageException("Added mileage should have later date than the latest in the record");
                else if(add.getMileage()<latest.getMileage())
                    throw new MileageException("Added mileage should have greater value of the mileage than the latest in the record");
                 else if(add.getMileage()<0)
                     throw new MileageException("Added mileage should have greater value of the mileage than zero");
                 else
                  mileages.add(add); 
            }
            else
                throw new MileageException("Added mileage should not be null");   
        }
    }
    ...
    public void tryReturn() throws Exception{
        if(rented==false)
            throw new Exception("The car is already returned");
        else
            this.setReturned();
    }
    ...
    public void changeRentPrice(float newPrice) throws Exception{
        if(newPrice<=0)
            throw new Exception("New price can not be less or equal to zero");
        else
            hourPrice=newPrice;
    }
    ...
    public void raiseRentPrice(float newPrice) throws Exception{
        if(newPrice<=0)
            ...
            throw new Exception("New price can not be less or equal to the previous price");
        else
            hourPrice=newPrice;
    }
    ...
    public void lowerRentPrice(float newPrice) throws Exception{
        if(newPrice<=0)
            ...
            throw new Exception("New price can not be greater or equal to the previous price");
        else
            hourPrice=newPrice;
    }
